package src.javainproduction._3;
...
public class Exercises {
    ...
    private static int calculateFactorial(int number) {
        //Fix this code
        int result = 1;
        for (int i = number; i >= 1; i--) { //changed from: for (int i = number; i >= 1; i -= 2)
            result = result * i;
        }
        return result;
    }
    ...
    private static String reverseString(String input) {
        ...
        String reversed = "";
        for (int i = input.length() - 1; i >= 0; i--) { //changed from: for (int i = input.length() - 1; i >= 1; i--)
            reversed += input.charAt(i); //changed from: reversed += reversed + input.charAt(i);
        }
        return reversed;
    }
    ...
    private static boolean isPositive(int num) {
        //Fix this code
            return num > 0; // changed from: num >= 0;
                            // when num = 0 the method was returning 0 >= 0 that is true
                            // now it returns 0 > 0 that is false
